...
EchoesServer.prototype.who = function(nick) {
    ...
    nicks = Object.keys(this.nicknames);
...
EchoesServer.prototype.ulist = function(id, rooms) {
    ...
    chans = rooms.splice(); // pass by value
...
EchoesServer.prototype.attach_events = function(client) {
    ...
    client.on('/echo', function(echo) {
        if (self.is_channel(echo.to)
            && self.is_in(client.nickname, echo.to)) {
    ...
    client.on('/ulist', function(args) {
        var chans = self.ulist(client.id, client.rooms);
    ...
    client.on('/who', function(args) {
        var nicks = self.who(client.nickname);

        self.io.to(client.id).emit('*who', { nicknames: nicks });
    });
